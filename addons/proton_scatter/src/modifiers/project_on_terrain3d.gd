...
@export_range(0.0, 90.0, 0.1) var min_slope = 90.0
...
func _process_transforms(transforms, domain, _seed) -> void:
	...
	var remapped_min_slope: float = remap(min_slope, 0.0, 90.0, 0.0, 1.0)
	for i in transforms.list.size():
		...
		if abs(Vector3.UP.dot(normal)) <= (remapped_min_slope):
			t.origin.y = gt.origin.y if is_nan(height) else height - gt.origin.y
			new_transforms_array.push_back(t)
